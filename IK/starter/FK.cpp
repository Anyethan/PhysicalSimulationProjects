...
FK::FK(const std::string & jointParentsFilename, const std::string & skeletonConfigFilename)
{
  ...
  int exitCode = ListIO::load(jointParentsFilename.c_str(), jointParents, listOffset, sortListAfterLoad); //this file shows each joint's parent using its index
  assert(exitCode == 0);
...
//      cout << "joint " << i << " " << orderStr << endl;
  ...
  vector<bool> jointVisited(numJoints, false); //joint ¸ö false as a list
  ...
  //jointUpdateOrder size == joint num
  std::function<void(int)> goToParent = [&](int jointID) -> void
  {
    ...
    {
      goToParent(jointParents[jointID]); //recursively 
    }
	...
	vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
	...
	{
		double R_ori[9];  //R_child_o
		euler2Rotation(jointOrientationEulerAngles[i], R_ori, rotateOrders[i]);
		double R_angle[9]; //R_child_local
		euler2Rotation(eulerAngles[i], R_angle, rotateOrders[i]);

		Mat3d M_child_local;
		for (int j = 0; j < 3; j++) {
			for (int k = 0; k < 3; k++) {
				M_child_local[j ][ k] = R_ori[3 * j] * R_angle[0 + k] + R_ori[3 * j + 1] * R_angle[1 * 3 + k] + R_ori[3 * j + 2] * R_angle[2 * 3 + k];
			}
		}		
		
		localTransforms[i] = RigidTransform4d(M_child_local, translations[i]);
	}

	for (int i = 0; i < eulerAngles.size(); i++) {
		Mat4d M_child_global;
		int parent = jointParents[jointUpdateOrder[i]];
		if( parent >= 0){
			M_child_global = globalTransforms[parent]* localTransforms[jointUpdateOrder[i]];
		}
		else {
			M_child_global = localTransforms[jointUpdateOrder[i]];
		}
		globalTransforms[jointUpdateOrder[i]] = M_child_global;
	 }
}
