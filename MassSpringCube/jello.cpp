...
int g_iMenuId;
int g_vMousePos[2];
int g_iLeftMouseButton,g_iMiddleMouseButton,g_iRightMouseButton, draging;
int mous_pos[3] = { 0 };
...
void dragForce(int x, int y) {
	
	if (mous_pos[0] != 10000) {//signal of failed selection
		ProcessDrag(&jello, mous_pos, x, y);
	}
}
void click(int x, int y) {
	ProcessClick(&jello, mous_pos, x, y);
