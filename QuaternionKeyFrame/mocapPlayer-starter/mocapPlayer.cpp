...
void RenderGroundPlane(double groundPlaneSize, double groundPlaneHeight, double rPlane, double gPlane, double bPlane, double ambientFskeleton, double diffuseFskeleton, double specularFskeleton, double shininess)
{
  ...
  for(int i=0; i<planeResolution; i++)
    for(int j=0; j<planeResolution; j++)
    {
      ...
      float factor = (((i+j) % 2) == 0) ? 0.2f : 0.9f;
      ...
      glBegin(GL_QUADS);
      glVertex3f((float)(-groundPlaneSize/3 + i * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize/3 + j * planeIncrement));
      glVertex3f((float)(-groundPlaneSize/3 + i * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize/3 + (j+1) * planeIncrement));
      glVertex3f((float)(-groundPlaneSize/3 + (i+1) * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize/3 + (j+1) * planeIncrement));
      glVertex3f((float)(-groundPlaneSize/3 + (i+1) * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize/3 + j * planeIncrement));
      glEnd();
    }
